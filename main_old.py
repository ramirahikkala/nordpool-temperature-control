...
SPOT_HINTA_SENSOR = os.getenv("SPOT_HINTA_SENSOR", "sensor.spot_hinta_prices")  # DEPRECATED - no longer used
SPOT_HINTA_API_JUSTNOW = os.getenv("SPOT_HINTA_API_JUSTNOW", "https://api.spot-hinta.fi/JustNow")  # Current price
SPOT_HINTA_API_URL = os.getenv("SPOT_HINTA_API_URL", "https://api.spot-hinta.fi/TodayAndDayForward")  # Today + tomorrow
...
def get_price_from_day_array(prices_array):
    ...
    Args:
        prices_array: List of 96 prices (one per quarter-hour, 00:00-23:45) in c/kWh
    ...
    if 0 <= quarter < len(prices_array):
        price_cents = prices_array[quarter]
        logger.debug(f"Quarter {quarter}: {price_cents:.2f} c/kWh")
        return price_cents
    else:
